...
for drop in [0, 0.05, 0.2, 0.5]:
    sens = estimator.Sensor(True, rate_hz=rate, isDrop=True, p_drop=drop)

    for i in range(500):

        sens.reseed(SENSOR_SEED + i, DROP_SEED + i)
        ...
        output[i] = {
            ...
            'obstacle_condition': [po_x, po_y, obs.po_r_true],
            'gap_norm': gap/obs.po_r_true,
            'offset_norm': scene.offset/obs.po_r_true,
            'min |h|': 0,
            'clearance': 0,
            'path length': 0,
            'max |a|': 0,
        ...
        output[i]['margin_breach'] = bool(d_min < obs.po_r_true + obs.delta)
        output[i]['max_age'] = sens.max_age
    ...
    with open(f'output_sensor_noiseTrue_inflated_rad_{delta}m_rate_{rate}Hz_pDrop_{int(drop*100)}.json', 'w', encoding='utf-8') as f:
